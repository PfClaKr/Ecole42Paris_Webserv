...
void	Socket::init_socket_fd_nonblocking()
{
	...
	if (fd_flag < 0)
		throw(perror("fcntl"), "fcntl");
	if (fcntl(this->fd, F_SETFL, O_NONBLOCK) < 0)
		throw(perror("fcntl nonblocking"), "fcntl nonblocking");
}
